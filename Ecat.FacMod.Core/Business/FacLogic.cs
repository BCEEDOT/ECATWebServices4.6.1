...
namespace Ecat.FacMod.Core
{
    ...
    public class FacLogic : IFacLogic
    {
        ...
        async Task<List<WorkGroup>> IFacLogic.GetRoadRunnerWorkGroups(int courseId)
        {

            var latestCourse = _repo.GetCourses.Where(crse => crse.Id == courseId).Single();

            var workGroupsInCourse = await(from workGroup in _repo.GetCourseWorkGroups
