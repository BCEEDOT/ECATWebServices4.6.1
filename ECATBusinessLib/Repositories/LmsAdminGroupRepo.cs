...
namespace Ecat.Business.Repositories
{
    ...
    public class GroupOps : ILmsAdminGroupOps
    {
        ...
        public async Task<SaveGradeResult> SyncCanvasGrades(int crseId)
        {
            ...
            var response = new HttpResponseMessage();
            var apiResource = "courses/" + groups[0].Course.BbCourseId + "/assignments?per_page=1000";
            ...
            var canvasEvalAssign = canvAssignments.Where(ca => ca.name == models.Last().StudStratCol).FirstOrDefault();
            ...
            gradeData = gradeData.TrimEnd(',');
            gradeData += "}}";

            var postApiResource =
                "courses/" + groups[0].Course.BbCourseId + "/assignments/" + canvasEvalAssign.id + "/submissions/update_grades";
      
            var postResponse = new HttpResponseMessage();

            
            postResponse = await CanvasOps.PostGrades(gradeData, postApiResource, canvasLogin);
            result.HasToken = true;

            if (!postResponse.IsSuccessStatusCode)
            {
                result.Success = false;
                result.Message = "There was an error attempting to post scores to the LMS";
                return result;
            }
    ...
    //    if (canvasLogin?.AccessToken == null)
    //    {
    //        var errorResult = new GroupMemReconResult();
    //        errorResult.HasToken = false;
    //        groupMemReconResults.Clear();
    //        groupMemReconResults.Add(errorResult);
    //        return groupMemReconResults;
    //    }

    //    var response = new HttpResponseMessage();
    //    var apiResource = "courses/" + courseWithWorkGroups.BbCrseId + "/sections?per_page=1000&include[]=students";
    ...
    //    if (canvasSectionsReturned == null)
    //    {
    ...
    //        CanvasBusinessLogic.ReconcileGroupMembers(courseWithWorkGroups, canvasSectionsReturned);

    //    //var membersToAdd = courseWithWorkGroups.WorkGroups.SelectMany(wg => wg.Members).Where(mem => mem.NewEnrollment).ToList();
    //    var membersToDelete = courseWithWorkGroups.WorkGroups.SelectMany(wg => wg.Members).Count(mem => mem.RemoveEnrollment);
    //    var membersToAdd = courseWithWorkGroups.WorkGroups.SelectMany(wg => wg.Members).Count(mem => mem.NewEnrollment);
    //    var membersToMove = courseWithWorkGroups.WorkGroups.SelectMany(wg => wg.Members).Count(mem => mem.IsMoving);

    //    if (membersToAdd > 0)
    //    {
    //        //groupMemReconResults = AddGroupMembers(courseWithWorkGroups, groupMemReconResults);
    //    }


    //    return new System.Collections.Generic.List<GroupMemReconResult>();
    //}
